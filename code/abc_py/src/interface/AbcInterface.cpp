...
bool AbcInterface::resub(IntType k, IntType n, IntType f, bool l, bool z, bool v)
{
...
bool AbcInterface::If(IntType C, bool v)
{
    std::string cmd = "if";
    if (C > 0)
    {
...
bool AbcInterface::lutpack(bool v)
{
    std::string cmd = "lutpack";
    if (v)
    {
        cmd += " -v";
    }
...
bool AbcInterface::mfs2(IntType W, IntType M, IntType p, bool l, bool v, bool w)
{
    std::string cmd = "mfs2";
    if (W > 0)
    {
...
bool AbcInterface::compress2rs()
{
    ...
    if (!this->resub(6, -1, -1, true, false))
    {
        return false;
    }
    if (!this->rewrite(true, false))
    {
        return false;
    }
    if (!this->resub(6, 2, -1, true, false))
    {
        return false;
    }
    if (!this->refactor(-1, true, false))
    {
        return false;
    }
    if (!this->resub(8, -1, -1, true, false))
    {
        return false;
    }
    if (!this->balance(true, false, false, false))
    {
        return false;
    }
    if (!this->resub(8, 2, -1, true, false))
    {
        return false;
    }
    if (!this->rewrite(true, false))
    ...
    if (!this->resub(10, -1, -1, true, false))
    {
        return false;
    }
    if (!this->rewrite(true, true))
    {
        return false;
    }
    if (!this->resub(10, 2, -1, true, false))
    {
        return false;
    }
    if (!this->balance(true, false, false, false))
    ...
    if (!this->resub(12, -1, -1, true, false))
    {
        return false;
    }
    if (!this->refactor(-1, true, true))
    {
        return false;
    }
    if (!this->resub(12, 2, -1, true, false))
    {
        return false;
    }
    if (!this->rewrite(true, true))
...
std::string AbcInterface::getStats()
{
    char Command[1000];
    sprintf(Command, "print_stats -t");
    ...
    // printf("end print stats command");
    fflush(stdout);
    ...
    std::string str(&string[0], end_index + 1);
...
AigStats AbcInterface::aigStats()
{
    ...
    for (IndexType idx = 0; idx < words.size(); ++idx)
    {
        ...
        if (words.at(idx) == "nd")
        {
            IntType numAnd = std::stoi(words.at(idx + 1));
            idx = idx + 3;
            stats.setNumAnd(numAnd);
        }
        if (words.at(idx) == "lev")
